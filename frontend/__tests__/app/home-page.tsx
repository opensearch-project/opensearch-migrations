...
describe("DefaultPage", () => {
  ...
  it("loads page if backend service is responsive from cache", async () => {
    ...
    render(<DefaultPage />);
    expect(screen.getByText(/Welcome to Migration Assistant/i)).toBeInTheDocument();
  ...
  it("routes to loading page if backend service is non-responsive, on timeout", async () => {
    ...
    server.use(
      http.get("http://localhost/system/health", async () => {
        ...
        return HttpResponse.json({status: "error", checks: {} }, {status: 503});
      })
    );
    ...
    expect(screen.getByText(/Welcome to Migration Assistant/i)).toBeInTheDocument();
    expect(setSiteReadiness).not.toHaveBeenCalled();
    await waitFor(() => expect(replaceMock).toHaveBeenCalledWith("/loading"), {timeout: 6_000});
  });
