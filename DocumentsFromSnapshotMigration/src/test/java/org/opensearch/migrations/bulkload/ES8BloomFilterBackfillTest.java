...
import org.opensearch.migrations.bulkload.common.*;
...
@Tag("isolatedTest")
public class ES8BloomFilterBackfillTest extends SourceTestBase {
    ...
    private static Stream<SearchClusterContainer.ContainerVersion> es8Sources() {
        return Stream.concat(
                        SupportedClusters.supportedPairs(false).stream()
                                .map(SupportedClusters.MigrationPair::source),
                        SupportedClusters.extendedSources().stream()
                )
                .filter(src -> VersionMatchers.isES_8_X.test(src.getVersion()))
                .distinct();
    }
    ...
    @ParameterizedTest(name = "ES 8.x {0} ➔ {1}")
    @MethodSource("scenarios")
    void bloomFilterBackfill(
