        ...
        post {
            always {
                eksPostCleanup(
                    ...
                    // isolated-vpc teardown + buildStack confirm run AFTER the MA
                    // stack delete — isolated-vpc tears down a VPC the MA stack
                    // owned, and the build stack was asynchronously deleted back in
                    // Phase 2 (we just confirm it's actually gone here).
                    stepsAfterMaDelete: [
                        isolatedVpcId ? [type: 'isolated-vpc', vpcId: isolatedVpcId, stage: env.maStageName] : null,
                        [type: 'cfn-destroy', stackName: buildStackName,
                         reason: 'build stack — delete initiated in Phase 2, confirming here'],
                    ].findAll { it },
