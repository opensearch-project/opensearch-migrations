...
/**
 * At a basic level, this class aims to be a generic serializer which can receive ByteBuffer data
 * and serialize the data into the defined Protobuf format
 * {@link org.opensearch.migrations.trafficcapture.protos.TrafficStream}, and then write this
 * formatted data to the provided CodedOutputStream.
 * <p>
 * Commented throughout the class are example markers such as (e.g. 1: "1234ABCD") which line up
 * with the textual representation of this Protobuf format to be used as a guide as fields are
 * written. An example TrafficStream can also be visualized below for reference.
 * <p>
 * 1: "91ba4f3a-0b34-11ee-be56-0242ac120002" 5: "5ae27fca-0ac4-11ee-be56-0242ac120002" 2 { 1 { 1:
 * 1683655127 2: 682312000 } 4 { 1: "POST /test-index/_bulk?pretty…” } } 2 { 1 { 1: 1683655127 2:
 * 683973000 } 15 { 1: 38 2: 105 } } 3: 1
 */
@Slf4j
public class StreamChannelConnectionCaptureSerializer<T> implements
    IChannelConnectionCaptureSerializer<T> {
  ...
  public StreamChannelConnectionCaptureSerializer(String nodeId, String connectionId,
    ...
    assert (nodeId == null ? 0
        : CodedOutputStream.computeStringSize(TrafficStream.NODEID_FIELD_NUMBER, nodeId)) +
  ...
  private CodedOutputStream getOrCreateCodedOutputStream() throws IOException {
    ...
    if (currentCodedOutputStreamHolderOrNull != null) {
      ...
      // e.g. 1: "1234ABCD"
      currentCodedOutputStream.writeString(TrafficStream.CONNECTIONID_FIELD_NUMBER,
          connectionIdString);
      ...
      if (eomsSoFar > 0) {
        currentCodedOutputStream.writeInt32(TrafficStream.PRIORREQUESTSRECEIVED_FIELD_NUMBER,
            eomsSoFar);
      }
  ...
  /**
   * Will write the beginning fields for a TrafficObservation after first checking if sufficient
   * space exists in the CodedOutputStream and flushing if space does not exist. This should be
   * called before writing any observation to the TrafficStream.
   */
  private void beginSubstreamObservation(Instant timestamp, int captureTagFieldNumber,
      int captureTagLengthAndContentSize) throws IOException {
    final var tsContentSize = CodedOutputStreamSizeUtil.getSizeOfTimestamp(timestamp);
    final var tsTagSize = CodedOutputStream.computeInt32Size(TrafficObservation.TS_FIELD_NUMBER,
        tsContentSize);
    final var captureTagNoLengthSize = CodedOutputStream.computeTagSize(captureTagFieldNumber);
    final var observationContentSize =
        tsTagSize + tsContentSize + captureTagNoLengthSize + captureTagLengthAndContentSize;
    // Ensure space is available before starting an observation
    if (getOrCreateCodedOutputStream().spaceLeft() <
        CodedOutputStreamSizeUtil.bytesNeededForObservationAndClosingIndex(observationContentSize,
            numFlushesSoFar + 1)) {
  ...
  private void writeTimestampForNowToCurrentStream(Instant timestamp) throws IOException {
    ...
    getOrCreateCodedOutputStream().writeUInt32NoTag(
        CodedOutputStreamSizeUtil.getSizeOfTimestamp(timestamp));

    getOrCreateCodedOutputStream().writeInt64(Timestamp.SECONDS_FIELD_NUMBER,
  ...
  private void writeByteBufferToCurrentStream(int fieldNum, ByteBuffer byteBuffer)
      throws IOException {
  ...
  public CompletableFuture<T> flushCommitAndResetStream(boolean isFinal) throws IOException {
    ...
    var fieldNum = isFinal ? TrafficStream.NUMBEROFTHISLASTCHUNK_FIELD_NUMBER
        : TrafficStream.NUMBER_FIELD_NUMBER;
    // e.g. 3: 1
    currentStream.writeInt32(fieldNum, ++numFlushesSoFar);
    log.trace("Flushing the current CodedOutputStream for {}.{}", connectionIdString,
        numFlushesSoFar);
    currentStream.flush();
    assert currentStream == currentCodedOutputStreamHolderOrNull.getOutputStream() :
        "Expected the stream that " +
  ...
  private void addDataMessage(int captureFieldNumber, int dataFieldNumber, Instant timestamp,
    ...
    int messageAndOverheadBytesLeft = CodedOutputStreamSizeUtil.maxBytesNeededForASegmentedObservation(
        timestamp,
    ...
    if (byteBuffer.limit() == 0
        || messageAndOverheadBytesLeft <= getOrCreateCodedOutputStream().spaceLeft()) {
      int minExpectedSpaceAfterObservation =
          getOrCreateCodedOutputStream().spaceLeft() - messageAndOverheadBytesLeft;
    ...
    while (byteBuffer.position() < byteBuffer.limit()) {
      ...
      int chunkBytes = messageAndOverheadBytesLeft > availableCOSSpace ? availableCOSSpace
          - trafficStreamOverhead : byteBuffer.limit() - byteBuffer.position();
  ...
  private void addSubstreamMessage(int captureFieldNumber, int dataFieldNumber,
      int dataCountFieldNumber, int dataCount,
    ...
    beginSubstreamObservation(timestamp, captureFieldNumber,
  ...
  public void addReadEvent(Instant timestamp, ByteBuf buffer) throws IOException {
    addDataMessage(TrafficObservation.READ_FIELD_NUMBER, ReadObservation.DATA_FIELD_NUMBER,
        timestamp, buffer);
  }

  @Override
  public void addWriteEvent(Instant timestamp, ByteBuf buffer) throws IOException {
    addDataMessage(TrafficObservation.WRITE_FIELD_NUMBER, WriteObservation.DATA_FIELD_NUMBER,
        timestamp, buffer);
  }
  ...
  private void writeEndOfHttpMessage(Instant timestamp) throws IOException {
    int eomPairSize =
        CodedOutputStream.computeInt32Size(EndOfMessageIndication.FIRSTLINEBYTELENGTH_FIELD_NUMBER,
            firstLineByteLength) +
            CodedOutputStream.computeInt32Size(
                EndOfMessageIndication.HEADERSBYTELENGTH_FIELD_NUMBER, headersByteLength);
    int eomDataSize = eomPairSize + CodedOutputStream.computeInt32SizeNoTag(eomPairSize);
    beginSubstreamObservation(timestamp, TrafficObservation.ENDOFMESSAGEINDICATOR_FIELD_NUMBER,
        eomDataSize);
    // e.g. 15 {
    ...
    getOrCreateCodedOutputStream().writeInt32(
        EndOfMessageIndication.FIRSTLINEBYTELENGTH_FIELD_NUMBER, firstLineByteLength);
    getOrCreateCodedOutputStream().writeInt32(EndOfMessageIndication.HEADERSBYTELENGTH_FIELD_NUMBER,
        headersByteLength);
  }
  ...
  private void writeEndOfSegmentMessage(Instant timestamp) throws IOException {
    ...
    getOrCreateCodedOutputStream().writeMessage(TrafficObservation.SEGMENTEND_FIELD_NUMBER,
        EndOfSegmentsIndication.getDefaultInstance());
  }

  private void observationSizeSanityCheck(int minExpectedSpaceAfterObservation, int fieldNumber)
      throws IOException {
    int actualRemainingSpace = getOrCreateCodedOutputStream().spaceLeft();
    if (actualRemainingSpace < minExpectedSpaceAfterObservation
        || minExpectedSpaceAfterObservation < 0) {
      log.warn(
          "Writing a substream (capture type: {}) for Traffic Stream: {} left {} bytes in the CodedOutputStream but we calculated "
              +
              "at least {} bytes remaining, this should be investigated", fieldNumber,
