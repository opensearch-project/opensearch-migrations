...
def send_request(index_suffix, no_refresh, multi_type=False, type_name=None):
    """Send a request using console clusters command."""
    ...
    command = f"""console clusters curl -XPUT source_cluster "{url_path}?refresh={refresh_param}" -H "Content-Type: application/json" -d '{payload}'"""
...
def parse_args():
    ...
    parser.add_argument("--requests-per-sec", type=float, default=10.0,
                        help="Target requests per second to be sent.")
    parser.add_argument("--no-refresh", action='store_true',
                        help="Flag to disable refresh after each request.")
...
def main():
    ...
    while True:
        ...
        if args.enable_multi_type:
            type_name = "type1" if total_requests % 2 != 0 else "type2"
            response_code, request_timestamp_or_error, response_json = send_request(
                current_index, args.no_refresh, multi_type=True, type_name=type_name)
        else:
            response_code, request_timestamp_or_error, response_json = send_request(
                current_index, args.no_refresh)
