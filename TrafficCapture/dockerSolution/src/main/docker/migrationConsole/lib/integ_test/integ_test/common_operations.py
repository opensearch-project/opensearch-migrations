...
def execute_api_call(cluster: Cluster, path: str, method=HttpMethod.GET, data=None, headers=None, timeout=None,
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            logger.debug(f"Attempt {attempt}: Calling API at {path} with method {method}...")
            response = call_api(cluster=cluster, path=path, method=method, data=data, headers=headers,
