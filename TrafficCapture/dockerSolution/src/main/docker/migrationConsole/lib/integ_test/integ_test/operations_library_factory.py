...
from .opensearch_operations import (
    OpensearchV1_XOperationsLibrary, OpensearchV2_XOperationsLibrary
)


def get_operations_library_by_version(version: ClusterVersion):
    if is_incoming_version_supported(
        limiting_version=ElasticsearchV1_X, incoming_version=version
    ):
        return ElasticsearchV1_XOperationsLibrary()
    elif is_incoming_version_supported(
        limiting_version=ElasticsearchV2_X, incoming_version=version
    ):
        return ElasticsearchV2_XOperationsLibrary()
    elif is_incoming_version_supported(
        limiting_version=ElasticsearchV5_X, incoming_version=version
    ):
        return ElasticsearchV5_XOperationsLibrary()
    elif is_incoming_version_supported(
        limiting_version=ElasticsearchV6_X, incoming_version=version
    ):
        return ElasticsearchV6_XOperationsLibrary()
    elif is_incoming_version_supported(
        limiting_version=ElasticsearchV7_X, incoming_version=version
    ):
        return ElasticsearchV7_XOperationsLibrary()
    elif is_incoming_version_supported(
        limiting_version=ElasticsearchV8_X, incoming_version=version
    ):
