...
class ClusterVersion:
    ...
    def __init__(self, version_str: str):
        ...
        if not match:
            raise ValueError(
                f"Invalid version format: {version_str}. Cluster versions must be in "
                f"format ES_x.y or OS_x.y, where y is a number or 'x' for any minor version."
            )

        self.cluster_type = match.group(1)
        self.full_cluster_type = (
            "elasticsearch" if self.cluster_type == "ES" else "opensearch"
        )
...
def is_incoming_version_supported(limiting_version: ClusterVersion,
                                   incoming_version: ClusterVersion):
