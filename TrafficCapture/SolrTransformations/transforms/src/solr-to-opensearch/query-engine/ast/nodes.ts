 ...
 *
 * Example:
 *   prettyPrint(BoolNode { and: [FieldNode(title,java), FieldNode(author,smith)] })
 *   → "title:java AND author:smith"
 *
 *   prettyPrint(BoostNode { child: FieldNode(title,java), value: 2 })
 *   → "title:java^2"
 */
export function prettyPrint(node: ASTNode): string {
  // TODO: implement — walk the AST and emit canonical Solr syntax
