...
#
# Spins up Solr 8, OpenSearch 3.3, and the TransformationShim proxy via
...
if [ "$SKIP_BUILD" = false ]; then
  ...
  step "Building TransformationShim Docker image (gradle jibDockerBuild)..."
...
step "Starting docker-compose (OpenSearch 3.3, Solr 8, TransformationShim proxy)..."
...
sleep 3
wait_for "Shim Proxy" "http://localhost:8080/_shim/health" 60
...
show_curl "Proxy health endpoint" \
  curl -s "http://localhost:8080/_shim/health"
...
echo ""
echo "  Solr Client  ──▶  TransformationShim Proxy (:8080)  ──▶  OpenSearch (:9200)"
echo "                         │                    ▲"
...
echo "  Proxy health:     http://localhost:8080/_shim/health"
echo ""
