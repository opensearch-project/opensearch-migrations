...
class ReplayTransactionTest {
    ...
    void resourceCleanupFailureRecordsAFailedTerminalOutcome() {
        ...
        Assertions.assertSame(cleanupFailure, failure.getCause());
        Assertions.assertEquals(
            List.of(ReplayTransaction.TerminalOutcome.FAILED),
            metrics.terminalOutcomes
        );
        Assertions.assertTrue(metrics.onlyMailboxCallbacks);
    }
    ...
    void explicitFailureRetainsOwnedRecordsSoTheLedgerStillQuiesces() {
        ...
        transaction.fail(new IllegalStateException("actor failed"));
        mailbox.runUntilIdle();
        ...
        Assertions.assertTrue(
            ledger.unresolvedObligations().toCompletableFuture().join().isEmpty(),
            "a failed transaction must not leave orphaned record obligations"
        );
    }
