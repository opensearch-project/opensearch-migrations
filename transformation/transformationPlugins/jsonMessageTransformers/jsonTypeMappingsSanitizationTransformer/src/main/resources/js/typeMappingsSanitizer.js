...
function convertSourceIndexToTargetViaRegex(sourceIndex, sourceType, regexMappings) {
    ...
    for (const { sourceIndexPattern, sourceTypePattern, targetIndexPattern } of regexMappings) {
        // Special handling: if sourceTypePattern is ".*", it should match any type including "_doc"
        ...
        if (conjoinedRegex.test(conjoinedSource)) {
            // Replace the entire matched string with the target pattern
            // This ensures that capture groups from the index pattern work correctly
            const result = conjoinedSource.replace(conjoinedRegex, targetIndexPattern);
            return result;
        }
    }
    return null;
...
function processBulkIndex(docBackfillPair, context) {
    ...
    // When _type is not present in the document, we should still try to match
    // against patterns. Use "_doc" as default when no type is specified.
...
function processBulkDocument(document, context) {
    const indexMetadata = document.get("index");
    const sourceIndexName = indexMetadata._index;
    const typeName = indexMetadata._type ?? "_doc";
    ...
    if (!targetIndex) return document;
    
    // Create a new index metadata object without _type and with the new index
    const newIndexMetadata = {};
    // Copy all existing fields except _type
    for (const key in indexMetadata) {
        if (key !== '_type') {
            newIndexMetadata[key] = indexMetadata[key];
        }
    }
    // Update the index to the target index
    newIndexMetadata._index = targetIndex;
    
    // Return the transformed document
    return new Map([
        ["index", newIndexMetadata],
...
function detectAndTransform(document, context) {
    ...
    if (document.has("type") && document.has("name") && document.has("body")) {
        ...
        // Handle bulk document format with index metadata and source
        return processBulkDocument(document, context);
    } else {
        return document;
    }
}
