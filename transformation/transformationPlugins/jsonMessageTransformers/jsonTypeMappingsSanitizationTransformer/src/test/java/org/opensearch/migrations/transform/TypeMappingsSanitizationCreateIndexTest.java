...
public class TypeMappingsSanitizationCreateIndexTest {
    ...
    @Test
    public void testMultiTypeIndexMergedPreservesDynamicFalse() throws Exception {
        final String index = "socialTypes";
        var testString = "{\n" +
            "  \"" + JsonKeysForHttpMessage.METHOD_KEY + "\": \"PUT\",\n" +
            "  \"" + JsonKeysForHttpMessage.PROTOCOL_KEY + "\": \"HTTP/1.1\",\n" +
        ...
        try (var indexTypeMappingRewriter = makeIndexTypeMappingRewriter()) {
            ...
            // The merged mappings should preserve dynamic=false
            Assertions.assertTrue(resultJson.contains("\"dynamic\":false"),
                "Merged create index should preserve dynamic=false, got: " + resultJson);
        }
    }

    @Test
    public void testSingleTypeCreateIndexPreservesDynamicFalse() throws Exception {
        ...
        try (var indexTypeMappingRewriter = makeIndexTypeMappingRewriter()) {
            ...
            String resultJson = OBJECT_MAPPER.writeValueAsString(result);
            Assertions.assertTrue(resultJson.contains("\"dynamic\":false"),
                "Single type create index should preserve dynamic=false, got: " + resultJson);
        }
    }
}
