...
VALID_SOURCE_VERSIONS = ["ES_1.5", "ES_2.4", "ES_5.6", "ES_6.8", "ES_7.10", "ES_8.19", "OS_1.3", "SOLR_6.6", "SOLR_7.7", "SOLR_8.11", "SOLR_9.8"]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=TargetType.OPENSEARCH.value,
        help=f"Target cluster type. Default: {TargetType.OPENSEARCH.value}. "
             f"Use '{TargetType.AOSS.value}' for Amazon OpenSearch Serverless (--target-version is ignored)."
    )
    parser.add_argument(
        "--target-version",
        ...
        help=f"Target version (only used when --target-type={TargetType.OPENSEARCH.value}). "
             f"Must be one of: {', '.join(VALID_TARGET_VERSIONS)}"
    )
    parser.add_argument(
        "--skip-delete",
        action="store_true",
