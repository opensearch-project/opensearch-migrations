 ...
 *
 * <p>{@code startOffset} and {@code endOffset} are -1 when the field was not indexed with
 * {@code index_options: offsets}. Callers must check for -1 before using offsets for
 * gap-preserving reconstruction.
 */
public record TermEntry(String term, int position, int startOffset, int endOffset) {

    /** Sentinel value for start/end character offsets when the field was not indexed with offsets. */
