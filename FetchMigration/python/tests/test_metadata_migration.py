...
def create_config_section(plugin_config: dict) -> dict:
    valid_plugin = dict()
    valid_plugin[random.choice(metadata_migration.SUPPORTED_ENDPOINTS)] = plugin_config
...
class TestMetadataMigration(unittest.TestCase):
    ...
    def test_is_insecure_default_value(self):
        self.assertFalse(metadata_migration.is_insecure({}))

    def test_is_insecure_top_level_key(self):
        test_input = {"key": 123, INSECURE_KEY: True}
        self.assertTrue(metadata_migration.is_insecure(test_input))
    ...
    def test_is_insecure_missing_nested(self):
        ...
        self.assertFalse(metadata_migration.is_insecure(test_input))
    ...
    def test_get_auth_returns_none(self):
        ...
        for test_input in input_list:
            self.assertIsNone(metadata_migration.get_auth(test_input))

    def test_get_auth_for_valid_input(self):
        # Test valid input
        result = metadata_migration.get_auth({"username": "user", "password": "pass"})
    ...
    def test_validate_plugin_config_unsupported_endpoints(self):
        ...
        self.assertRaises(ValueError, metadata_migration.validate_plugin_config, BASE_CONFIG_SECTION, TEST_KEY)

    def test_validate_plugin_config_missing_host(self):
        test_data = create_config_section({})
        self.assertRaises(ValueError, metadata_migration.validate_plugin_config, test_data, TEST_KEY)
    ...
    def test_validate_plugin_config_auth_disabled(self):
        ...
        metadata_migration.validate_plugin_config(test_data, TEST_KEY)
    ...
    def test_validate_pipeline_config_missing_required_keys(self):
        ...
        for config in bad_configs:
            self.assertRaises(ValueError, metadata_migration.validate_pipeline_config, config)

    def test_validate_pipeline_config_happy_case(self):
        # Get top level value
        test_config = next(iter(self.loaded_pipeline_config.values()))
        metadata_migration.validate_pipeline_config(test_config)
