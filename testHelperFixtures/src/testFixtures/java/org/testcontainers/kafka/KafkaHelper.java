...
class KafkaHelper {
    ...
    static Map<String, String> envVars() {
        ...
        envVars.put("KAFKA_CONTROLLER_QUORUM_VOTERS", envVars.get("KAFKA_NODE_ID") + "@localhost:9094");
    ...
    static void resolveListeners(GenericContainer<?> kafkaContainer, Set<String> listenersSuppliers) {
        Set<String> listeners = Arrays.stream(kafkaContainer.getEnvMap().get("KAFKA_LISTENERS").split(","))
        ...
        for (int i = 0; i < listenersToTransform.size(); i++) {
            ...
            kafkaContainer.withNetworkAliases(listenerHost);
        }

        kafkaContainer.getEnvMap().put("KAFKA_LISTENERS", String.join(",", listeners));
        kafkaContainer.getEnvMap().put("KAFKA_LISTENER_SECURITY_PROTOCOL_MAP",
            String.join(",", listenerSecurityProtocolMap));
    }
    ...
    static List<String> resolveAdvertisedListeners(Set<Supplier<String>> listenerSuppliers) {
        ...
        for (int i = 0; i < listenersToTransform.size(); i++) {
            String protocol = String.format("%s-%d", PROTOCOL_PREFIX, i);
            String listener = listenersToTransform.get(i).get();
            advertisedListeners.add(String.format("%s://%s", protocol, listener));
        }
