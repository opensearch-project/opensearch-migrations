...
# Global state for cancellation support
_current_workflow_url = None
_jenkins_url = None
_pipeline_token = None


def _cancel_jenkins_build(signum, frame):
    ...
    if _current_workflow_url:
        try:
            stop_url = f"{_current_workflow_url}stop"
            headers = {"Authorization": f"Bearer {_pipeline_token}"} if _pipeline_token else {}
...
def wait_for_job_completion(jenkins_url: str, trigger_job_response_body: dict, job_name: str,
    ...
    while result_status is None and total_wait_time <= timeout_seconds:
        ...
        if workflow_url:
            _current_workflow_url = workflow_url
...
def trigger_and_wait_for_job(jenkins_url: str, pipeline_token: str, payload: dict, target_job_name: str,
    ...
    global _jenkins_url, _pipeline_token
    _jenkins_url = jenkins_url
    _pipeline_token = pipeline_token
