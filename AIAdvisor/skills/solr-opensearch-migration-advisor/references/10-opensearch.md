...
- **Expect BM25 scores to differ from Solr's TF-IDF scores.** OpenSearch defaults to BM25 (`k1=1.2`, `b=0.75`); Solr historically defaulted to classic TF-IDF. Score values will change — this is expected and generally improves relevance. Re-validate any application logic that uses raw score thresholds (e.g. "only show results with score > 0.5").
...
- **Replace eDisMax `pf`/`pf2`/`pf3` phrase boosts with `multi_match` phrase clauses.** There is no direct equivalent to Solr's phrase field boosting. Approximate it by adding a `multi_match` query with `type: phrase` in a `bool.should` clause alongside the main query.
...
- **Replace Solr `solrconfig.xml` request handler defaults with index-level settings and templates.** Default search parameters configured in `solrconfig.xml` (e.g. default `rows`, `fl`, `sort`) must move to the application layer or be encoded in the query body. OpenSearch has no server-side default query parameter configuration.
...
- **Use the `_reindex` API with `slices: auto` for large datasets.** `slices: auto` parallelizes the reindex across primary shards, which is usually optimal. For the initial migration from Solr, use an external tool (Data Prepper, Logstash) rather than `_reindex`, which only copies between OpenSearch indexes.
...
- **Restore snapshots to a separate domain for validation.** Never test a restore on the production cluster. Spin up a temporary domain, restore the snapshot, validate the data and query results, then tear it down.
