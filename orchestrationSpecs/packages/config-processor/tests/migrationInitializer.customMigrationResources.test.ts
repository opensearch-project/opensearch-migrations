...
describe("migration initializer CRD resource generation", () => {
    it("generates CR resources and UID enrichment script entries with expected names", async () => {
        ...
        expect(byKind("SnapshotMigration")).toContain("source-target-snap1-slice-0");
        ...
        expect(byKind("ApprovalGate")).toEqual(expect.arrayContaining([
                "evaluatemetadata.source-target-snap1-slice-0",
                "migratemetadata.source-target-snap1-slice-0",
            ...
            // SnapshotMigration CR reconcile VAP retry gate
                "snapshotmigration.source-target-snap1-slice-0.vapretry",
        ...
        expect(getResource("SnapshotMigration", "source-target-snap1-slice-0")?.spec.dependsOn).toBeUndefined();
        expect(getResource("TrafficReplay", "target-replay")?.spec.dependsOn).toEqual([
            "source-proxy",
            "source-target-snap1-slice-0",
        ]);
        expect(getResource("ApprovalGate", "evaluatemetadata.source-target-snap1-slice-0")?.spec.dependsOn,).toBeUndefined();
        expect(getResource("ApprovalGate", "migratemetadata.source-target-snap1-slice-0")?.spec.dependsOn,).toBeUndefined();
        ...
        expect(getResource("SnapshotMigration", "source-target-snap1-slice-0")?.spec.metadataMigrationEnableSourcelessMigrations,).toBe(false);
        ...
        expect(
            preflightByKindAndName("SnapshotMigration", "source-target-snap1-slice-0"
        ...
        expect(enrichScript).toContain(
            "snapshot_migration_source_target_snap1_slice_0=\"$(run_kubectl get snapshotmigrations.migrations.opensearch.org/source-target-snap1-slice-0 -o jsonpath='{.metadata.uid}')\"",
        );
        expect(enrichScript).toContain("snapshotMigrations: {");
        expect(enrichScript).toContain(
            '"source-target-snap1-slice-0": $snapshot_migration_source_target_snap1_slice_0',);
        expect(enrichScript).toContain(
    ...
    it("labels approval gates with workflow name and generates cleanup script", async () => {
        ...
        const migGate = gates.find((g: any) => g.metadata.name === "snapshotmigration.source-target-snap1-slice-0.vapretry",);
        expect(migGate.metadata.labels).toEqual({
            ...
            [MigrationInitializer.GATE_LABEL_RESOURCE_NAME]: "source-target-snap1-slice-0",
            ...
            [MigrationInitializer.GATE_LABEL_MIGRATION]: "slice-0",
        });
        const evaluateGate = gates.find(
            (g: any) => g.metadata.name === "evaluatemetadata.source-target-snap1-slice-0"
        );
        ...
        const backfillGate = gates.find(
            (g: any) => g.metadata.name === "documentbackfill.source-target-snap1-slice-0"
        );
