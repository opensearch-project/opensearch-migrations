...
(args: {
    ...
    // GCS: no test-creds volume (no LocalStack equivalent). Workload Identity supplies
    // credentials via the pod's KSA→GSA binding, not via mounted secrets.
    const finalContainerDefinition = setupLog4jConfigForContainer(
        useCustomLogging,
...
export const DocumentBulkLoadGcs = WorkflowBuilder.create({
    k8sResourceName: "document-bulk-load-gcs",
    serviceAccountName: "argo-workflow-executor"
})
