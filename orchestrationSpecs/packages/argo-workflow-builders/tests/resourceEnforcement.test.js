...
describe('Resource Enforcement', function () {
    it('should allow container with resources and verify resources block', function () {
        ...
        var testTemplate = rendered.spec.templates.find(function (t) { return t.name === 'test'; });
    ...
    it('should show compile-time error for container without resources', function () {
        ...
        src_1.WorkflowBuilder.create({
            ...
            .addTemplate('test', (function (t) { return t.addContainer(
    ...
    it.skip('should merge multiple resource specifications and verify merged result', function () {
        var wf = src_1.WorkflowBuilder.create({
            k8sResourceName: 'test-workflow',
            serviceAccountName: 'default'
        })
        ...
        var resourcesStr = testTemplate.container.resources;
        var expected = '{{=sprig.merge({"kind":"function","functionName":"sprig.merge","args":[' +
            '{"kind":"literal","value":{}},' +
    ...
    it.skip('should work with expression-based resources and verify in output', function () {
        var wf = src_1.WorkflowBuilder.create({
            k8sResourceName: 'test-workflow',
            serviceAccountName: 'default'
        })
            .addParams({
            cpuLimit: { description: 'CPU limit' }
        })
            .addTemplate('test', function (t) { return t
            .addContainer(function (c) { return c
            .addImageInfo('nginx:latest', 'IfNotPresent')
            .addResources(src_1.expr.literal(EXAMPLE_RESOURCES))
            .addCommand(['echo', 'hello']); }); })
            .getFullScope();
        expect(wf).toBeDefined();
        expect(wf.templates.test).toBeDefined();
