...
const HELP = `
Usage: build-unified-schema [--output <file>] [--strimzi-openapi <file>] [--base-only]
...
Arguments:
  ...
  --base-only               Build a base-only schema with no Strimzi defs (for external-Kafka deployments).
`.trim();
