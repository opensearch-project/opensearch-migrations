...
The `examples/cdc-load-test/deployCdcLoadTestConfig.sh` helper is the one to reach for when you want a
running capture proxy and replayer locally, with k6 ready to drive traffic through them. If a CDC
deployment and CaptureProxy already exist, skip this example and run `installK6Chart.sh install`
directly.

What it submits is a migration **config** (`examples/cdc-load-test/cdcLoadTest.yaml`, or your own
with `-f`), not a workflow: it feeds the config to `workflow configure edit --stdin` +
`workflow submit` in the migration console, and the console's config processor generates the Argo
workflow from it. The script then waits on the resulting CRs and installs the k6 chart. It is a
driver over the supported path rather than an alternate deployment mechanism — the same two console
...
```bash
./examples/cdc-load-test/deployCdcLoadTestConfig.sh up --dry-run   # show the config that would be submitted
./examples/cdc-load-test/deployCdcLoadTestConfig.sh up             # submit + wait for the CRs to report Ready + install k6
./examples/cdc-load-test/deployCdcLoadTestConfig.sh up --no-auth   # ... against clusters deployed with valuesNoAuth.yaml
./examples/cdc-load-test/deployCdcLoadTestConfig.sh status         # CR phases, workflow phase, proxy endpoint
./examples/cdc-load-test/deployCdcLoadTestConfig.sh down           # delete the migration resources + the k6 chart
```
