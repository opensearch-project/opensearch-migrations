...
Configuration is one of the greatest aspects to how users interact with the migration tools - especially the ones that run as services.  For example, what's the address of the source cluster, how fast should the replayer replay data, and what transformations does the user want to run on their data?

The applications included in this Migrations repository make few assumptions about the environments that they're running in and are generally configured via command line options.  If parameters are to change, those applications will need to be restarted to pick up the new changes.  Kubernetes can facilitate handling that sort of task  easily and gracefully.

It's up to the deployment charts in this package to wire the customer's intentions to those applications.  Configurations are generally stored within [ConfigMaps](https://kubernetes.io/docs/concepts/configuration/configmap/) and [Secrets](https://kubernetes.io/docs/concepts/configuration/secret/) (none yet, but we'll eventually need to).  Component charts share templates to create **initialization containers** that convert those Kubernetes-specific configurations into the containers, which are initialized directly from the migration images that are vended for any solution.  That initialization container's behavior pulls all configurations that each application/container needs to care about into environment variables, including one that represents the arguments that can be passed to the invocation of the appliaction.  Those variables are then packaged into a file on a share-mounted directory that is shared with the main container.  The main container for each application (migration-console included) then sources the initialized file into its own shell before invoking the main command with the $ARGS variable that it had just sourced.  Notice that additional other environment variables are also available so that containers can easily discover other configurations without needing to parse command line arguments (e.g. is the destination using https; is the password included; etc).

To specify values, first consult the individual charts' READMEs to understand what values can be configured and what each of them does.  Notice that when installing an aggregate chart, configurations for individual components must be rooted in a key with the name of the component.  For example, the following shows part of the values.yaml from the root to configure the replayer component of the migrationAssistant chart.
...
The migrationAssistant chart constructs **shared** values that multiple pods may utilize so that common values can be shared across the kubernetes cluster.  Users don't need to worry about changing what values should be shared across applications, but it is helpful to understand that shared values are defined in one place so that when a value is adjusted in a shared configuration, it may have an impact on multiple applications.  Like per-application values, shared values are stored and retrieved via ConfigMaps or Secrets.  Unlike values that are defined within each application's values, these shared configurations are stored in a single configuration that's owned by a separate chart (for the migrationAssistant chart, the shared configurations are in the "shared-configs" component chart).

Both shared and per-application configuration values can be specified via a **default** configuration that is set at the **time of deployment** or a **runtime** one that can be set through the console (eventually) at any time.  When doing a deployment, the values passed to the installation/update will specify configurations.  Each configuration, for shared or per-application, will include an `allowRuntimeOverride` flag to specify if an additional ConfigMap should be setup in addition to the default one so that a value can be overridden.  Notice that this behavior will be inherited like any other one, so if not set, it will default to what the migrationAssistant or component chart specified.  In some cases, a runtime override may be critical (transforms, speedup factor) and in others, it may not make much sense (sourceCluster settings).
...
Notice that both the per-application configurations _and_ the shared configurations can be marked for overrides or not.  Notice too that the contents of the ConfigMaps will be structured differently for per-application configurations and shared ones.  
