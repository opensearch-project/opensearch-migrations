...
- **Import-VPC does not create VPC endpoints** — subnets must have NAT/IGW routes for public image pulls. The create-VPC template provisions ECR, S3, CloudWatch Logs, and EFS endpoints; the import-VPC template does not.
...
- **`public.ecr.aws` has no VPC endpoint** — even with ECR VPC endpoints, public ECR images require internet access. Use `--build-images` with private ECR for fully private subnets.
