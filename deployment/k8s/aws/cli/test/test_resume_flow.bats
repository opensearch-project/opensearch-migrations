...
@test "first run prompts the mode picker" {
  # No MODE in state yet → the CLI must show the AI/Manual picker.
  run bash -c "printf '1\n' | timeout 8 '$CLI_BIN' resume 2>&1"

  if [[ "$output" == *"unknown mode"* ]]; then
    _diag "first run: 'unknown mode' bug regression"
    return 1
  fi
  if [[ "$output" != *"How do you want to drive"* ]]; then
    _diag "first run: mode picker missing"
    return 1
  fi
}

@test "Mode=1 (Manual) progresses past mode-select" {
  run bash -c "printf '1\n' | timeout 8 '$CLI_BIN' resume 2>&1"
  if [[ "$output" == *"unknown mode"* ]]; then
  ...
  if [[ "$output" != *"OpenSearch Migration Assistant"* ]]; then
    _diag "Mode=Manual: banner missing"
    return 1
  fi
}
