...
The OTEL metrics collector configuration can be modified through `metrics.collectorConfig`. The trace collector is configured separately through `tracing.collectorConfig` and only runs when `tracing.enabled` is true.


#### AWS X-Ray traces
Traces, as a default, are not enabled for the EKS deployment configured by `valuesEks.yaml` in this directory. Enable `tracing.enabled` and set `tracing.backend: xray` to export traces to AWS X-Ray.
```yaml
    processors:
