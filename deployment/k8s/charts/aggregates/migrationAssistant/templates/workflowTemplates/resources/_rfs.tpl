...
spec:
  replicas: {{ "{{" }}=jsonpath(inputs.parameters['runtime-options'], '$.replicas') {{ "}}" }}
  selector:
    matchLabels:
  ...
  template:
    ...
    spec:
      containers:
        - name: bulk-load
          image: {{ "{{" }}=jsonpath(inputs.parameters['runtime-options'], '$.image'){{ "}}" }}
          imagePullPolicy: IfNotPresent
          env:
            - name: DOCUMENTS_PER_BULK_REQUEST
              value: {{ "{{" }}=jsonpath(inputs.parameters['runtime-options'], '$.documents-per-bulk-request'){{ "}}" }}
            - name: INITIAL_LEASE_DURATION
              value: {{ "{{" }}=jsonpath(inputs.parameters['runtime-options'], '$.lease-duration'){{ "}}" }}
            - name: LUCENE_DIR
              value: {{ "{{" }}=jsonpath(inputs.parameters['runtime-options'], '$.lucene-dir'){{ "}}" }}
            - name: SNAPSHOT_LOCAL_DIR
              value: {{ "{{" }}=jsonpath(inputs.parameters['runtime-options'], '$.snapshot-local-dir'){{ "}}" }}
            - name: SNAPSHOT_NAME
              value: {{ "{{" }}=jsonpath(inputs.parameters['runtime-options'], '$.snapshot-name'){{ "}}" }}
            - name: TARGET_HOST
              value: {{ "{{" }}=jsonpath(inputs.parameters['runtime-options'], '$.target-host'){{ "}}" }}
            - name: TARGET_INSECURE
              value: {{ "{{" }}=jsonpath(inputs.parameters['runtime-options'], '$.target-insecure'){{ "}}" }}
            - name: TARGET_PASSWORD
              value: {{ "{{" }}=jsonpath(inputs.parameters['runtime-options'], '$.target-password'){{ "}}" }}
            - name: TARGET_USERNAME
              value: {{ "{{" }}=jsonpath(inputs.parameters['runtime-options'], '$.target-username'){{ "}}" }}
          command:
            - "/bin/sh"
            - "-c"
            - |
