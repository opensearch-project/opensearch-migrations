...
kubectl -n {namespace} exec "${CONSOLE_POD}" -- bash -lc \
  "console clusters curl --path '/_cluster/health?pretty'"
```
...
- After the CFN stack completes, you MUST run `aws-bootstrap.sh` to install MA on the EKS cluster. This script reads the CFN exports, installs Helm, and runs `helm install` with the correct configuration.

### Bootstrap Script Parameters

The `aws-bootstrap.sh` script has many parameters that change across releases. Rather than hardcoding them, you MUST discover parameters at runtime:

```bash
# 1. Download the script
curl -sL https://github.com/opensearch-project/opensearch-migrations/releases/latest/download/aws-bootstrap.sh -o /tmp/aws-bootstrap.sh
chmod +x /tmp/aws-bootstrap.sh

# 2. Read its --help to discover all current parameters and descriptions
/tmp/aws-bootstrap.sh --help

# 3. Construct the invocation using the discovered parameters and the context
#    you have gathered (stage, region, VPC, subnets, etc.) from previous steps.
#    At minimum you will typically need: --stage, --region, and --skip-cfn-deploy
#    (since the CFN stack was already deployed above).
/tmp/aws-bootstrap.sh --skip-cfn-deploy --stage "<stage>" --region "${aws_region}"
```

**Constraints:**
- You MUST always read `--help` before invoking the script — do not assume parameters from memory.
- You MUST use `--skip-cfn-deploy` when the CFN stack was already deployed in a previous step.
- You MUST pass `--stage` and `--region` based on what was discovered/chosen earlier in this SOP.
- If the user requested a specific version, pass `--version <tag>`.
- Review the `--help` output for TLS, VPC endpoint, image, and access options — apply any that match the user's requirements.
...
kubectl -n {namespace} exec "${CONSOLE_POD}" -- bash -lc \
  "console clusters curl --path '/' --cluster source"

kubectl -n {namespace} exec "${CONSOLE_POD}" -- bash -lc \
  "console clusters curl --path '/_cluster/health?pretty' --cluster source"

# Target cluster health and version
kubectl -n {namespace} exec "${CONSOLE_POD}" -- bash -lc \
  "console clusters curl --path '/' --cluster target"

kubectl -n {namespace} exec "${CONSOLE_POD}" -- bash -lc \
  "console clusters curl --path '/_cluster/health?pretty' --cluster target"
```
...
kubectl -n {namespace} exec "${CONSOLE_POD}" -- bash -lc \
  "console clusters curl --path '/_cat/indices?v&h=index,health,status,pri,rep,docs.count,store.size&s=store.size:desc' --cluster source"

# Node configuration
kubectl -n {namespace} exec "${CONSOLE_POD}" -- bash -lc \
  "console clusters curl --path '/_cat/nodes?v&h=name,ip,heap.percent,ram.percent,cpu,load_1m,node.role,master' --cluster source"

# Shard count (for RFS worker calculation in Step 7)
kubectl -n {namespace} exec "${CONSOLE_POD}" -- bash -lc \
  "console clusters curl --path '/_cat/shards?h=index,shard,prirep&s=index' --cluster source" | grep ' p$' | wc -l
```
...
kubectl -n {namespace} exec "${CONSOLE_POD}" -- bash -lc \
  "console clusters curl --path '/<index>/_mapping' --cluster source"
```
