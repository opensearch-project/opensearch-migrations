...
import org.opensearch.migrations.bulkload.version_es_5_6.OpenSearchWorkCoordinator_ES_5_6;
...
public class WorkCoordinatorFactory {
    ...
    public OpenSearchWorkCoordinator get(
        ...
        if (VersionMatchers.isOS_1_X.or(VersionMatchers.isOS_2_X).or(VersionMatchers.isES_7_X).test(version)) {
            ...
            return new OpenSearchWorkCoordinator_ES_5_6(httpClient, tolerableClientServerClockDifferenceSeconds, workerId);
        } else {
    ...
    public OpenSearchWorkCoordinator get(
        ...
        if (VersionMatchers.isOS_1_X.test(version) || VersionMatchers.isOS_2_X.test(version) || VersionMatchers.isES_7_X.test(version)) {
        ...
        } else if (VersionMatchers.isES_6_X.test(version) || VersionMatchers.isES_5_X.test(version)) {
            ...
            return new OpenSearchWorkCoordinator_ES_5_6(httpClient, tolerableClientServerClockDifferenceSeconds, workerId, clock, workItemConsumer);
        } else {
            throw new IllegalArgumentException("Unsupported version: " + version);
        }
    }
