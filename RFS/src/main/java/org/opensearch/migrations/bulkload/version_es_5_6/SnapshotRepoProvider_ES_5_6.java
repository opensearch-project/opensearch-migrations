...
public class SnapshotRepoProvider_ES_5_6 implements SnapshotRepo.Provider {
    private final SourceRepo repo;
    private SnapshotRepoData_ES_5_6 repoData = null;

    public SnapshotRepoProvider_ES_5_6(SourceRepo repo) {
        this.repo = repo;
    }

    protected SnapshotRepoData_ES_5_6 getRepoData() {
        if (repoData == null) {
            this.repoData = SnapshotRepoData_ES_5_6.fromRepo(repo);
        }
        return repoData;
    }

    public List<SnapshotRepoData_ES_5_6.Index> getIndices() {
        return getRepoData().getIndices().entrySet()
            .stream()
            .map(entry -> SnapshotRepoData_ES_5_6.Index.fromRawIndex(entry.getKey(), entry.getValue()))
    ...
    public List<SnapshotRepo.Index> getIndicesInSnapshot(String snapshotName) {
        ...
        SnapshotRepoData_ES_5_6.Snapshot targetSnapshot = getRepoData().getSnapshots().stream()
        ...
        if (targetSnapshot != null) {
            getRepoData().getIndices().forEach((indexName, rawIndex) -> {
                if (rawIndex.getSnapshots().contains(targetSnapshot.getId())) {
                    matchedIndices.add(SnapshotRepoData_ES_5_6.Index.fromRawIndex(indexName, rawIndex));
                }
            });
        }
    ...
    public String getSnapshotId(String snapshotName) {
        for (SnapshotRepoData_ES_5_6.Snapshot snapshot : getRepoData().getSnapshots()) {
