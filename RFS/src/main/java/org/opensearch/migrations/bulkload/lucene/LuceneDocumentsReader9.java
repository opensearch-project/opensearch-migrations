...
public class LuceneDocumentsReader9 implements LuceneDocumentsReader {
    ...
    static class SegmentNameSorter implements Comparator<LeafReader> {
        static final LuceneDocumentsReader9.SegmentNameSorter INSTANCE = new LuceneDocumentsReader9.SegmentNameSorter();
        @Override
     ...
     *
     * @param originalLeaves A list of {@link shadow.lucene9.org.apache.lucene.index.LeafReaderContext} representing the document segments.
     ...
     *         wrapped in {@link org.opensearch.migrations.bulkload.lucene.LuceneDocumentsReader9.ReaderAndBase}.
     */
    public static Flux<LuceneDocumentsReader9.ReaderAndBase> getSegmentsFromStartingSegment(List<shadow.lucene9.org.apache.lucene.index.LeafReaderContext> originalLeaves, int startDocId) {
        ...
        var sortedLeaves = originalLeaves.stream()
            ...
            .sorted(LuceneDocumentsReader9.SegmentNameSorter.INSTANCE)
            .collect(Collectors.toList());

        // Step 2: Build the list of ReaderAndBase objects with cumulative doc base
        var sortedReaderAndBase = new ArrayList<LuceneDocumentsReader9.ReaderAndBase>();
        int cumulativeDocBase = 0;
        for (var segment : sortedLeaves) {
            sortedReaderAndBase.add(new LuceneDocumentsReader9.ReaderAndBase(segment, cumulativeDocBase));
        ...
        var segmentStartingDocIds = sortedReaderAndBase.stream().map(LuceneDocumentsReader9.ReaderAndBase::getDocBaseInParent).toArray();
     ...
     */
    Publisher<RfsLuceneDocument> readDocsByLeavesFromStartingPosition(shadow.lucene9.org.apache.lucene.index.DirectoryReader reader, int startDocId) {
    ...
    Flux<RfsLuceneDocument> readDocsFromSegment(LuceneDocumentsReader9.ReaderAndBase readerAndBase, int docStartingId, Scheduler scheduler,
                                                int concurrency) {
    ...
    protected shadow.lucene9.org.apache.lucene.index.DirectoryReader wrapReader(DirectoryReader reader, boolean softDeletesEnabled, String softDeletesField) throws IOException {
    ...
    protected RfsLuceneDocument getDocument(IndexReader reader, int luceneDocId, boolean isLive, int segmentDocBase, final Supplier<String> getSegmentReaderDebugInfo) {
        Document document;
        try {
            document = reader.storedFields().document(luceneDocId);
        } catch (IOException e) {
