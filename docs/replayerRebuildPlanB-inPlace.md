...
| `B7`+`B8` | **One merge unit.** Demand model (`N = P * T_threads`, one assignment bootstrap entitlement plus at most one intake-issued request per partition generation, demand recomputed after every intake input, empty polls resolving neither entitlement nor reaching intake) *and* removal of `KafkaRecordOwnershipBudget`. See the note above. | `D15`; `R4`, `R5`, `R7`, `R18` |
