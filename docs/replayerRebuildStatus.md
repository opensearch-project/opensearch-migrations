...
| Phase traceability | Baseline executable mainline behavior at `2fe4538a`; trace only moved, split, merged, substantially rewritten, or retired responsibilities with exact paired `OldClass.oldMethod -> NewClass.newMethod` records. A post-baseline method may be a target only for the inherited responsibility slice it receives; unchanged responsibilities and branch-only responsibilities with no mainline predecessor receive no trace. Calibrate each phase on 3–5 owner-confirmed samples before bulk editing; retain records through later phases and remove only in final pre-merge cleanup | owner decided, 2026-09-25 | G5 through final |
...
| Broad G5 `REBUILD-TRACE` prose predates the exact-method convention | G5 | Audit every existing record against mainline baseline `2fe4538a`; retain only exact paired mappings for moved, split, merged, substantially rewritten, or retired behavior. The committed calibration set maps transformation-context ownership from `HttpJsonTransformingConsumer` to `RequestReplayOwner` and removes traces from unchanged `RequestPipelineOrchestrator` and `NettySendByteBufsToPacketHandlerHandler` |
