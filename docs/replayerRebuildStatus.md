...
| G3 | `blocked(PA2)` | The typed producer → Kafka → rebuilt intake → dumper chain is implemented and its core evidence passes. The owner resolved early final responses as retained and unowned typed interims as ignored; closure waits on implementation of those rulings and the remaining truncated-interim-at-close disposition |
...
| Typed source-interim protocol | owner decision | core chain proved; two edge rulings designed, one pending | Typed whole/segmented observations flow through the real chain with no compatibility path. Pre-EOM final responses are retained, unowned interims are ignored, and the truncated-at-close disposition remains open |
...
| Source interim protocol | Typed whole/segmented observations based on PR #3000; ordinary `Write` is never a fallback and old captures get no compatibility decoder | core chain proved; early-final/unowned-interim design amended; truncated-at-close ruling pending | PA2/G3 |
...
|---|---|---|
| G3 closure | blocked(PA2) | The typed producer/consumer chain and interoperability are proved; implement the early-final/unowned-interim ruling and resolve the truncated-interim-at-close disposition |
...
| Ordinary final source `Write` before request EOM | resolved by owner, 2026-09-25; implementation pending | Retain exact whole/segmented bytes and associations until EOM allocates the request identity, then attach as that request's final response |
| Typed source interim with no current request | resolved by owner, 2026-09-25; implementation pending | Ignore during inherited-tail discard or between requests and never attach it to the next request |
