...
| PA2 | `blocked(proxy hardening decisions)` | All five focused repairs and G3 interoperability are proved. Closure still waits on the candidate source-response-header memory-bound decision and next-request telemetry-failure behavior recorded below |
...
| Typed source-interim settled edge semantics | proved | Incomplete headers become ordered ordinary final writes; pre-EOM final responses are retained; unowned typed interims are ignored. Five mutations failed at `b08be26e7`, telemetry isolation failed when removed at `f1832b5b0`, and the final production review found no design-conformance defect |
...
|---|---|---|
| G3 closure | resolved, 2026-09-25 | Typed producer/consumer interoperability and all three settled edge semantics are proved; the remaining PA2 decisions are proxy-local and do not reopen G3 |
| PA2 closure | blocked(proxy hardening decisions) | Select the candidate source-response-header memory-bound behavior and next-request telemetry-failure behavior below |
...
| Candidate source informational header never reaches `CRLF CRLF` | open; blocks PA2 | Choose unbounded per-connection classifier buffer, a configured source-header bound that flushes ordinary final bytes and closes the connection, or a provisional wire encoding; review recommends the configured bound |
| Telemetry failure while ending a response at the next-request boundary | open; blocks PA2 | Choose best-effort continuation, current diagnostic exception/connection close, or required-capture failure; review recommends the same best-effort handling used by terminal close |
