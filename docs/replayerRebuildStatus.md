...
| G8 | proved | Milestone evidence is complete through `6b8d38bce`, with production complete through `240678951`: typed revocation and unbounded shutdown grace, generation-scoped force and cleanup, every-predecessor successor gating, independent pause reasons, tracker-gauge conservation, and protocol-violation termination are wired through the real top-level chain. The final eight-class focused set passed 105 tests in `/private/tmp/gradle-evidence.39777.18867.log`; exact production compile passed in `/private/tmp/gradle-evidence.19723.31649.log`. The direct-CLI falsification batch caught every final repaired mutation and restored clean at `6b8d38bce`. Claude session `replayer-G8-20260925T231639Z-df3c` resumed through the final evidence/register correction and returned `NO_ACTIONABLE_FINDINGS` |
| G9 | proved | Production is complete and pushed through `f4be4f7b3`, with the archive preserve-only design change isolated in `9bc7ed96a`: deployed startup retains the G5–G8 owners and queues, constructs all 33 preserved semantic metric producers and indexed tuple workers, validates the complete CLI/inline-JSON surface, and separates G8 orderly drain from supervised fatal exit/watchdog behavior. Exact compile passed in `/private/tmp/gradle-evidence.11985.24721.log`; 57 focused unit/construction tests passed in `/private/tmp/gradle-evidence.12429.14321.log`; inherited Kafka/replayer integration evidence passed in `/private/tmp/gradle-evidence.63785.7804.log` and `/private/tmp/gradle-evidence.79592.17406.log`. The final direct-CLI falsification batch caught all three mutations and restored clean production source, and the same named Claude session returned `NO_ACTIONABLE_FINDINGS` on `f4be4f7b3` |
| Production compile | proved | G9 exact production checkpoint `f4be4f7b3` passed with the required Gradle Spotless exclusions in `/private/tmp/gradle-evidence.11985.24721.log` |
| Compiled test set | proved | G9 focused unit/construction set: 57 tests, 0 failures, 0 errors, 0 skipped in `/private/tmp/gradle-evidence.12429.14321.log`; inherited isolated Kafka/replayer confirmation passed in `/private/tmp/gradle-evidence.63785.7804.log` and same-connection order passed in `/private/tmp/gradle-evidence.79592.17406.log`; the inherited full historical suite remains outside the progress oracle |
| Limbo regions | open | Measured 2026-09-26: 172 files with `REBUILD-LIMBO-START`; historical counts drift and the START grep is authoritative |
...
|---|---|---|---|---|---|---|
| R1 | One named owner per mutable value | G9 | procCommit §3.1; replayerLLD §2; async §2 | connLLD §19.6; procCommit §13.1 | proved | `KafkaSourceOwner`, `ReplayIntakeOwner`/`PartitionIntakeState`, `TargetConnectionOwner`, `RequestReplayOwner`, one event-loop-confined `TupleWriter` per configured worker, and `ProcessSupervisor` retain distinct mutable state; deployed composition adds no replacement owner or parallel cancellation model |
...
| R7 | Finished/cancelled cannot re-enter supply | G6, G7, G8 | kafkaLLD §12, §13; procCommit §8.1 | kafkaLLD §17.4 | proved | G8 typed cancellation invokes G7's shared `finishedOrCancelled` transition once before cleanup; queued normal completion and later retry input cannot remove or re-add supply |
...
| R15 | Cancellation cleanup cannot produce a commit request | G8 | replayerLLD §6; kafkaLLD §15.3; connLLD §17.2 | connLLD §19.5; kafkaLLD §17.5 | proved | Typed owner cleanup emits only `GenerationCleanupFinished`; deterministic evidence observes no `RecordProcessingFinished` or commit authority from discarded trackers and associations |
| R16 | Successor generation waits for prior cleanup | G8 | kafkaLLD §15.3; procCommit §9.3 | kafkaLLD §17.5 | proved | Deterministic source-owner evidence holds the newest generation behind every still-cleaning predecessor, ignores wrong-generation acknowledgement, and resumes after the last exact cleanup |
| R17 | Unrelated partitions continue | G8 | kafkaLLD §5.1; procCommit §8 | kafkaLLD §17.4, §17.5 | proved | Deterministic source-owner evidence delivers an unrelated partition while another partition remains paused for generation cleanup |
...
| R19 | Every unexpected owner failure reaches the supervisor | G9 | replayerLLD §8, §4; async §2 | connLLD §19.6; procCommit §13.5 | proved | Kafka source run/rebalance callbacks, replay intake, permit and outstanding-operation invariants, target connection/request owners, tuple workers, target event-loop termination, composition cleanup, protocol-terminator close, and application sequencing all deliver owner + operation + failure to the one-shot `ProcessSupervisor` |
...
| D13 | Fatal path halts instead of supervisor ladder | G9 | proved | First signal wins; input stops; diagnostics flush; deployed `System.exit(80/89)` runs with a ten-minute watchdog that writes the thread dump and invokes same-code `Runtime.halt` |
| D14 | Termination waits for orderly recovery | G9 | proved | `runReplayMode`, `setupShutdownHookForReplayer`, and `awaitReplayerShutdown` are replaced by `SupervisedReplayApplication`; fatal termination skips the orderly intake fence and every potentially dead owner/group wait, while normal shutdown alone enters G8's unbounded host-bounded drain |
...
| Active-record-tracker gauge during generation cancellation | A, unreachable before G8 | proved | Generation cleanup removes every unfinished tracker and decrements `replayIntakeActiveRecordTrackers` once per tracker without counting ordinary retirement or adding identity attributes; deterministic evidence returns the process-wide gauge to its prior value |
...
## G8 latest dispositions

| Finding / decision | Class | State | Required disposition |
|---|---|---|---|
| Revocation cancellation sequence and Kafka-thread behavior | conformance | proved | `KafkaSourceOwner.onPartitionsRevoked` performs the designed eight steps: lifecycle close, typed graceful submission, eligible commit/lifecycle processing while waiting on one monotonic deadline, unconditional force submission, return after intake acceptance, and asynchronous cleanup; no record batch is admitted in the callback |
| Shutdown grace | owner-authorized design change | proved | Every active or callback-assigned generation receives `CancellationGrace.Shutdown`; intake stops, incomplete source assembly is released, complete admitted requests drain, tuples flush eagerly, eligible commits are attempted promptly, no timer or force upgrade is created, and close remains host-bounded until cleanup and commit resolution finish |
| Cleanup condition 1: source assembly | conformance | proved | Typed cancellation removes incomplete request/response assembly and stale connection reconstruction before a successor generation can apply records |
| Cleanup condition 2: connection owners | conformance | proved | Every registered connection owner returns typed cleanup or ordinary shutdown completion for its exact `ConnectionProcessingId`; stale or duplicate post-cleanup acknowledgements are diagnostic and cannot mutate live state |
| Cleanup condition 3: requests and tuples | conformance | proved | Each request and tuple reaches ordinary completion or typed cleanup through its connection owner; cancellation is proved before preparation, while queued, during active target work, during retry/source-response waits, and during tuple durability |
| Cleanup condition 4: timers and permits | conformance | proved | Generation-scoped retry timers, target-attempt permits, target channels, and owner operations are released before local cleanup completion; shutdown grace creates no force timer |
| Cleanup condition 5: required completion inputs | conformance | proved | `GenerationCleanupTracker` cannot complete while a required request, owner, or cleanup acknowledgement remains unapplied; exact-generation idempotence and a late shutdown assignment in the close gate are proved |
| Cleanup condition 6: trackers and associations | conformance / observability | proved | Cleanup removes every unfinished record tracker and request association, decrements `replayIntakeActiveRecordTrackers` exactly once per removed tracker, returns the process-wide gauge to its prior value, and emits no generation-valued metric attribute |
| Commit authority under cancellation | R15 / G4 preservation | proved | Cleanup results emit only `GenerationCleanupFinished`; discarded trackers cannot emit `RecordProcessingFinished`, create a commit request, or advance an ineligible position |
| Retry-ready supply cancellation | R7 | proved | Each cancellation-ended target turn invokes G7's shared `finishedOrCancelled` transition once before cleanup; counted supply is removed once and queued completion or late retry input cannot re-add it |
| Successor and unrelated-partition behavior | R16 / R17 | proved | A successor remains paused until every earlier local generation has reported cleanup, while demand, lifecycle, and cleanup pause reasons remain independent and unrelated partitions continue reading |
| Protocol violation | conformance | proved | The violating record becomes commit-ineligible, blocks its offset and every later offset, pauses intake, permits a fixed 60-second side-effect drain, and terminates with exit `81`; restart rereads the poison record, and a late violation for a cleaned generation is diagnostic only |
| Inherited cancellation evidence and trace | navigation | proved | Exact paired G8 mappings retain the five inherited test classes' applicable source methods in limbo and point to typed stale-release, generation-scoped acknowledgement, shared-supply, and every-predecessor gating evidence; `TrafficSourceReaderInterruptedClose` remains unrestored |
| G8 responsibility placement | conformance | proved | Grace identity: `identity.CancellationGrace`; source producer, wait, commit, pause, shutdown, and protocol latch: `kafkasource.KafkaSourceOwner`; business/source queues: `intake.ReplayIntakeInput` and `kafkasource.KafkaSourceInput`; generation cleanup/supply/tracker bookkeeping: `intake.PartitionIntakeState` and `intake.ReplayIntakeOwner`; stale assembly release: `intake.SourceConnectionState`; request/connection cancellation: `lifecycle.RequestReplayOwner` and `lifecycle.TargetConnectionOwner`; eager output: `sink.TupleWriter`; violation drain: `ProtocolViolationTerminator`; construction: `TrafficReplayerTopLevel`; metrics: `ReplayIntakeMetrics` and `ProtocolViolationMetrics` |
| G8 falsification | test fidelity | proved | Fifteen final-check mutations failed. Three initially passing mutations exposed missing assertions for late shutdown assignment in the close gate, every earlier generation rather than one predecessor, and real default-constructor wiring; exact `ea5cc43b2` reruns caught all three in `/private/tmp/gradle-evidence.27633.32081.log`, `/private/tmp/gradle-evidence.28291.4668.log`, and `/private/tmp/gradle-evidence.29437.20831.log`. Final review found that case 28 still combined bootstrap and explicit demand; the bootstrap-only exact-revision rerun caught both premature first-predecessor release and missing bootstrap in `/private/tmp/gradle-evidence.46928.10289.log` and `/private/tmp/gradle-evidence.47507.9343.log`, then restored clean at `6b8d38bce` |
| G8 conformance review | conformance | proved | Same-session Claude review fixed shutdown revoke/loss force upgrade, assignment during shutdown, active-generation impossible-input handling, one-second default wiring, cleanup metric semantics, and the G9 `System.exit` handoff note; production re-review returned `NO_ACTIONABLE_FINDINGS`. The `ea5cc43b2` pass found no production defect and two low evidence/register findings: make case 28 bootstrap-only and stop assigning a design-silent intake-failure rule to G9. Both were corrected in `6b8d38bce`; the same named session resumed by persisted session ID `0d79d231-c026-4b26-a851-86597f82980a` and returned `NO_ACTIONABLE_FINDINGS` |

## G9 latest dispositions

| Finding / decision | Class | State | Required disposition |
|---|---|---|---|
| Process supervisor's six duties | R19 / conformance | proved | `ProcessSupervisor` accepts only the first signal, records owner and operation, invokes the abrupt input stopper, emits metric/log/stderr diagnostics and flushes them, starts deployed `System.exit`, and arms the ten-minute thread-dump/`Runtime.halt` watchdog before hooks can block |
| Fatal process termination | D13 / conformance | proved | Target event-loop owner loss uses code 80; other unexpected owner failure uses code 89. Both skip the orderly intake fence, close source/intake submission immediately, wake Kafka, enter no unbounded wait or doubling loop, and never join an event-loop group that may be dead |
| Orderly shutdown and intake failure during drain | G8 preservation / owner decision | proved | Normal shutdown alone invokes G8's unbounded host-bounded drain with no deadline, force upgrade, or competing cancellation. If replay intake fails during that drain, the owner-selected rule aborts the orderly wait and enters fatal supervision |
| Unexpected-owner-failure routes | R19 / conformance | proved | Kafka source run/assignment/revocation/loss, replay intake, permit provider, request and connection owners, outstanding-operation registries, tuple writers, target-loop termination, composition cleanup, protocol-terminator close, and application startup/source/shutdown sequencing all capture owner + operation and reach the same supervisor |
| Deployed CLI and inline JSON | compatibility | proved | Preferred `--max-concurrent-target-attempts` / `--maxConcurrentTargetAttempts`; deprecated `--max-concurrent-requests`, `--maxConcurrentRequests`, and workflow `spec.maxConcurrentRequests` map identically and conflicting new/old workflow values fail. Every supplied lookahead, quiescent, packet-timeout, or observed-packet alias parses, warns once, and is ignored. File input/`-t`, ownership caps, liveness-disable keys, and every unknown key fail immediately |
| Configuration defaults and validation | owner decision / conformance | proved | Code preview defaults remain 24 bytes; Java speed remains 1.0 and workflow speed remains 1.1; `E=30s`, `S=5s`, `W=5s`, `P=2`, revocation grace 1s. Startup rejects `P < 1`, target threads `< 1`, `W <= 0`, `E <= 0`, `S < 0`, and nonpositive speed |
| Archive configuration | owner-authorized design change | proved | Only `preserve` exists. Imported archives must retain trustworthy source broker numeric timestamps on the dedicated producer-timestamp topic; startup/import rejects missing, untrusted, or unretainable timestamps. `timestampMode` and `rebase-without-expiration` are absent |
| Real deployed replay construction | R1 / construction | proved | `TrafficReplayer.runReplayMode` constructs the existing Kafka source, source and intake queues, intake/partition state, connection/request owners, target channels, tuple workers, identities, typed links, and G8 shutdown chain under `SupervisedReplayApplication`; no replacement owner, queue, registry, or cancellation model was introduced |
| All 33 preserved semantic metric producers | D-4 / deployed construction | proved | Startup selects `kafkaRecordsRead`, `kafkaBytesRead`, `trafficStreamsRead`; `parsedHeader`, `parsedPayload`, `parsedPayloadSuccess`; `transformedJsonRequired`, `transformedJsonSucceeded`, `transformedTextSucceeded`, `transformedTextFailed`, `transformedPayloadBinary`; `originalPayloadBytesIn`, `uncompressedBytesIn`, `uncompressedBytesOut`, `finalPayloadBytesOut`; `transformSuccess`, `transformSkipped`, `transformError`, `transformBytesIn`, `transformBytesOut`, `transformChunksIn`, `transformChunksOut`; `scheduleLag`, `numRetriedRequests`, `lagBetweenSourceAndTargetRequests`; `activeReplayerChannels`, `nonRetryableConnectionFailures`; `activeTargetConnections`, `connectionsOpened`, `connectionsClosedCount`, `bytesWrittenToTarget`, `bytesReadFromTarget`; and `tupleComparison`, retaining shipped units, attributes, and parser/transform/physical-channel/byte/comparison events |
| Progress and tuple log outputs | compatibility / placement | proved | `ResultsToLogsConsumer.createTupleAndReportProgress` preserves `TransactionSummaryLogger` progress and inherited summary formatting over `RequestResult`; its physical worker sink preserves `OutputTupleJsonLogger`. Transformation remains worker-owned, progress metadata is not added to tuple/S3 output, disabled INFO does not serialize, and the inherited formatter was surgically adapted rather than replaced |
| Tuple-writer worker placement | G5 deferral / conformance | proved | The configured positive target-thread count creates that many workers exactly once each. Every stable integer index reaches transformer/sink construction and S3 naming; one event loop exclusively invokes each worker's transformer and sink; both close explicitly; connection construction only selects a worker and cannot construct or close per-connection tuple resources |
| Retry construction | owner decision | proved | Target retry classification remains internal standard-versus-bulk URI classification. Physical tuple writes use full-jitter exponential retry beginning at 100ms and capped at 5s |
| Successful preparation fallback | review correction | proved | Pass-through fallback with replayable bytes is `RequestPreparationReady` with completed status; filter skip alone has no prepared request. Ready-construction failure closes the prepared/output owner and completes exceptionally so `RequestReplayOwner` reaches fatal supervision instead of retaining an unresolved preparation. The identity-transformer test forces the null-reason redrive branch and passed in `/private/tmp/gradle-evidence.4167.27461.log` |
| Inherited long-running evidence | test restoration | proved | `KafkaCommitsWorkBetweenLongPollsTest`, `KafkaTrafficCaptureSourceLongTermTest`, capacity-one global replay, and same-connection captured-order evidence run against the rebuilt production chain; the same-connection helper waits for request/tuple count rather than Kafka-record count |
| Exact responsibility trace correction | navigation | proved | G9 retains the owner-confirmed shutdown, worker-construction/closure, and response-postprocessing mappings. The stale `packageAndWriteTuple -> deployedTupleFactory` claim is replaced by the exact split to `ResultsToLogsConsumer.createTupleAndReportProgress` and `RequestReplayOwner.tryStartTuple`; the public helper remains unclaimed because deployed startup does not select it |
| Rewrite audit | structure / efficiency | proved | `OpenSearchDefaultRetry` now promotes the inherited streaming bulk analyzer and changes only typed retry adapters. `ResultsToLogsConsumer` retains inherited logger names, formatter/helper structure, default selection, and disabled-logger behavior; its remaining rewrite is the required `SourceTargetCaptureTuple` → `RequestResult` and progress-versus-worker-output split. Other large G9 diffs own net-new startup, supervision, or worker composition |
| Async filter metric wording | plan defect | proved(design wins) | Asynchronous `RequestFilteredException` emits the shipped `transformSkipped` semantic producer as required by procCommit §7.5; Plan A prose implying otherwise is stale and does not override the design |
| Test-only `deployedTupleFactory` | low / structure | registered | The public helper remains test-reachable but is not a deployed construction path and carries no inherited-responsibility trace; deletion would be a separate public-contract decision |
| G9 conformance review | conformance | proved | Claude session `replayer-G9-20260926T140248Z-40b59d9a3` corrected preparation completion/ownership, target-loop exit classification, exact tuple trace placement, and the initially inert pass-through-fallback test. The same session's exact detached `f4be4f7b3` pass verified the identity-transformer test reaches null-reason redrive, would fail if completed status regressed to skipped, and returned `NO_ACTIONABLE_FINDINGS`. Its `SILENT` notes concern unchanged inherited worker-count default and response-postprocessor failure behavior plus the already owner-selected full-jitter policy; G9 assigns no new rule to them |
| G9 falsification | test fidelity | proved | One batched direct ephemeral Codex worker reversed response-postprocessing order, widened configured capacity one to `N+1`, and swapped queued requests on one connection. The narrowed tests failed respectively because the processed marker was absent (`/private/tmp/gradle-evidence.99336.11292.log`), the second request started while the permit was held (`/private/tmp/gradle-evidence.99965.3927.log`), and target order became `0,2,…,15,1` (`/private/tmp/gradle-evidence.1083.2442.log`). Every mutation was restored and the detached worktree finished clean at production source `05634940a`; `f4be4f7b3` changes only the independently reviewed fallback test |
| G9 responsibility placement | conformance | proved | Process sequencing: `TrafficReplayer.SupervisedReplayApplication`; fatal boundary: `ProcessSupervisor`; composition/input stop/worker pool: `TrafficReplayerTopLevel`; Kafka ownership: `KafkaSourceOwner`; intake and broker-time state: `ReplayIntakeOwner`/`PartitionIntakeState`; connection/request state: `TargetConnectionOwner`/`RequestReplayOwner`; output durability: `TupleWriter`; progress/log sink: `ResultsToLogsConsumer`; semantic telemetry: existing replay contexts and Netty/parser producers |
...
| G8 cancellation ports | Correctness-critical generation cancellation and cleanup methods are mandatory port methods rather than compatibility defaults | owner decided 1a, 2026-09-25 | G8 |
| Protocol-violation exit code | Use exit code `81`, distinct from fatal infrastructure exit `80`, after the fixed 60-second side-effect drain | owner decided 2a, 2026-09-25 | G8 |
| G8 trace calibration | Apply the approved representative exact source/target mappings, then trace only the inherited cancellation responsibility slices; keep their marked source members through the final completeness sweep | owner decided 3a, 2026-09-25 | G8 |
...
| Existing deprecated-option “parse-and-warn” adapter set does not exist | G9 | proved — every required lookahead/quiescent/observed-packet alias parses into a nullable compatibility field, emits one ignored-value warning, and reaches no owner, timer, demand, expiration, or cleanup behavior |
| Five of the 33 preserved names are pinned in three `capture-replay-dashboard.json` copies, not the k6 dashboard | G9 | proved — deployed startup constructs the complete 33-name semantic producer set, including the five dashboard-pinned names, without owner-transition approximations |
...
| Replace temporary per-connection tuple-writer placement and preserve stable sink index | G5 | G9 | G5 owns the complete logical transform/drop/retry/durability chain; G9 owns the configured bounded worker set, stable integer worker/sink index including S3 naming, per-worker transformer/sink construction, explicit close, and deletion of G5's per-connection placement | proved — one worker per configured target event loop owns one transformer and sink, receives its stable index at construction, is invoked only on that event loop, and closes explicitly; connection construction only selects the existing worker |
...
| Inherited revocation, stale-assembly, cleanup-acknowledgement, and successor-gating assertions | G3 | G8 | Requires typed generation cancellation and cleanup chain | proved — exact G8 traces retain the marked source methods and point to deterministic typed stale-release, cleanup-acknowledgement, shared-supply-transition, and every-predecessor gating tests |
| Active-record-tracker gauge cleanup balance | G3 | G8 | Only G8 can remove unfinished trackers during generation cancellation; decrement once per removal and prove return to the pre-generation value | proved — one decrement per removed tracker returns the process-wide gauge to its prior value without generation-valued attributes |
| Orderly-shutdown grace producer and drain chain | G9 | G8 | Owner moved the typed shutdown-grace producer, eager tuple flushing, prompt commit drain, and host-bounded completion into G8; G9 retains deployed startup/configuration, fatal supervision/watchdog, and tuple-writer worker placement | proved for G8 — shutdown grace is unbounded, never force-upgraded, eagerly flushes tuples, promptly attempts eligible commits, and closes only after cleanup and commit resolution; G9 retains only its named configuration/supervision/worker-placement scope |
...
| Cancellation invokes the G7 retry-ready supply exit | G7 | G8 | G8 owns typed cancellation and cleanup; G7 owns the shared count transition | proved — graceful/force cancellation reaches the shared `finishedOrCancelled` transition once, and queued normal completion or late retry input cannot change supply again |
| §17.4 case 28: bootstrap waits behind prior cleanup | G2 | G8 | G8 owns generation cleanup gate | proved — the newest assignment's bootstrap alone remains pending and paused until every earlier local generation still cleaning up reports exact cleanup, then reads without another demand signal |
...
| Marked revocation/stale-assembly/cleanup members | G2 carry | G8 | proved for cancellation responsibilities — exact paired traces retain the inherited methods in limbo and point to typed generation cancellation/cleanup evidence; process-teardown-only members remain G11 |
...
| Marked long-running Kafka/replayer integration tests | G2 carry | G9 | proved — the Kafka long-poll commit and capture-source long-term counterparts run on the rebuilt source, and the global replay evidence covers capacity-one target attempts plus same-connection order |
...
|---|---|---|
| `kafkaRecordsRead` / `kafkaBytesRead` / `trafficStreamsRead` | Shipped source-read event and byte contracts | G5 semantic producer; G9 deployed construction |
| `parsedHeader` / `parsedPayload` / `parsedPayloadSuccess` | Shipped request parse contracts | G5 semantic producer; G9 deployed construction |
| `transformedJsonRequired` / `transformedJsonSucceeded` / `transformedTextSucceeded` / `transformedTextFailed` / `transformedPayloadBinary` | Shipped request transformation-path contracts | G5 semantic producer; G9 deployed construction |
| `originalPayloadBytesIn` / `uncompressedBytesIn` / `uncompressedBytesOut` / `finalPayloadBytesOut` | Shipped transformation byte contracts | G5 semantic producer; G9 deployed construction |
| `transformSuccess` / `transformSkipped` / `transformError` / `transformBytesIn` / `transformBytesOut` / `transformChunksIn` / `transformChunksOut` | Shipped transformation result and stream contracts | G5 semantic producer; G9 deployed construction |
| `scheduleLag` / `numRetriedRequests` / `lagBetweenSourceAndTargetRequests` | Shipped pacing and retry contracts | G5 semantic producer; G9 deployed construction |
| `activeReplayerChannels` / `nonRetryableConnectionFailures` | Shipped replay-connection lifecycle contracts | G5 semantic producer; G9 deployed construction |
| `activeTargetConnections` / `connectionsOpened` / `connectionsClosedCount` / `bytesWrittenToTarget` / `bytesReadFromTarget` | Shipped target-connection and I/O contracts | G5 semantic producer; G9 deployed construction |
| `tupleComparison{sourceStatusCode,targetStatusCode,statusCodesMatch,method}` | Shipped fixed-cardinality source/target comparison contract | G5 semantic producer; G9 deployed construction |
...
| `replayIntakeGenerationGraceStarted{graceMode}` / `replayIntakeGenerationForceStarted` / `replayIntakeGenerationCleanupFinished` | Fixed-cardinality generation cancellation and cleanup transitions | G8 |
| `replayIntakeStaleGenerationInputsIgnored{inputKind}` | Diagnostic late input after cancellation or completed cleanup; identity is excluded | G8 |
| `replayProtocolViolationDrainsStarted` / `replayProtocolViolationTerminationsTriggered` | Fixed 60-second poison-record drain and exit-81 termination path | G8 |
...
|---|---|---|
| 2026-09-26 | `BringYourOwnCapturedTraffic` timestamp preservation, orchestration, and verification; `captureArch §2.3`, §8, §14, §15 | Remove `rebase-without-expiration` and the timestamp-mode option. Imported archives must always retain trustworthy archived source-broker timestamp values on a dedicated producer-timestamp topic; import/startup fails if the values cannot be retained or trusted. Replay always applies archived `E` and `S` and never substitutes destination-broker time |
...
| Intake owner fails during orderly shutdown drain | resolved and implemented, 2026-09-26 | Owner selected fail-fast fatal termination: abort the orderly wait, stop new input, and enter the supervisor ladder without manufacturing successful drain completion |
...
| Tuple-writer parallelism setting | resolved and proved in G9 | Worker count remains the configured positive target thread count; each worker is event-loop confined, owns one transformer/sink, and closes explicitly |
