...
| G8 | open(final review) | Production is complete through `240678951`: typed revocation and unbounded shutdown grace, generation-scoped force and cleanup, every-predecessor successor gating, independent pause reasons, tracker-gauge conservation, and protocol-violation termination are wired through the real top-level chain. The final eight-class focused set passed 105 tests in `/private/tmp/gradle-evidence.39777.18867.log`; exact production compile passed in `/private/tmp/gradle-evidence.19723.31649.log`. Direct-CLI falsification caught the three repaired close-gate, every-predecessor, and real-default-wiring mutations at `ea5cc43b2`; final bootstrap-only case-28 falsification remains. Claude session `replayer-G8-20260925T231639Z-df3c` found no production defect and two low evidence/register findings; both are corrected here pending same-session confirmation |
...
| G8 falsification | test fidelity | open(final case-28 rerun) | Fifteen final-check mutations failed. Three initially passing mutations exposed missing assertions for late shutdown assignment in the close gate, every earlier generation rather than one predecessor, and real default-constructor wiring; exact `ea5cc43b2` reruns caught all three in `/private/tmp/gradle-evidence.27633.32081.log`, `/private/tmp/gradle-evidence.28291.4668.log`, and `/private/tmp/gradle-evidence.29437.20831.log`. Final review then found that case 28 still combined bootstrap and explicit demand; the test is now bootstrap-only and awaits exact-revision falsification |
| G8 conformance review | conformance | open(final pass) | Same-session Claude review fixed shutdown revoke/loss force upgrade, assignment during shutdown, active-generation impossible-input handling, one-second default wiring, cleanup metric semantics, and the G9 `System.exit` handoff note; production re-review returned `NO_ACTIONABLE_FINDINGS`. The `ea5cc43b2` pass found no production defect and two low evidence/register findings: make case 28 bootstrap-only and stop assigning a design-silent intake-failure rule to G9. Both are corrected pending confirmation |
