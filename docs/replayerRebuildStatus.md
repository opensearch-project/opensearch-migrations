...
| 2026-09-24 | `kafkaLLD §9`; `procCommit §5.2` | A source write observed before the captured request's end-of-message marker is treated as an informational response such as `100 Continue`: replay intake ignores the write observation, or its complete segmented sequence, and continues assembling the request without advancing the ordinal or creating response state. | The owner chose option 1a on 2026-09-24 after the G3 review found the designs silent. The current implementation discarded the incomplete request and early write, silently losing a valid `Expect: 100-continue` request; the carried predecessor ignored the early write and preserved request assembly. |
...
| `SourceConnectionState` does not directly store the `contributing record identities` named by `kafkaLLD §9` | A | owner ruled — implementation follow-through open | `PartitionIntakeState` remains the sole source of contributing-record identity and reverse-association data; `SourceConnectionState` must not duplicate it. The owner chose one source of truth over redundant state |
...
| Observations after a known captured close | C | owner ruled — implementation follow-through open | Treat as a capture-protocol violation while the closed connection tombstone is still available; detection after tombstone retirement is best effort |
| Request timing evidence | C | owner ruled — implementation follow-through open | Preserve request first-byte source time, request-EOM source time, and request-completing Kafka `LogAppendTime`; each answers a different pacing or retry/expiration question |
| `SegmentEnd` with no active segmented value | C | owner ruled — implementation follow-through open | Ignore it and complete the record normally. The serializer can flush before the marker, so restart may legitimately expose the marker after preceding segment records were committed away |
