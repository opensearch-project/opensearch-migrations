...
| Production compile | proved | G8 exact production source `240678951` passed with the required Gradle Spotless exclusions in `/private/tmp/gradle-evidence.19723.31649.log` |
| Compiled test set | proved | G8 focused set: 105 tests, 0 failures, 0 errors, 0 skipped in `/private/tmp/gradle-evidence.39777.18867.log`; the inherited full historical suite remains outside the progress oracle |
...
| Intake owner fails during orderly shutdown drain | open owner decision at G9 boundary; design `SILENT` | The cited shutdown and supervision sections do not define whether or how `TrafficReplayerTopLevel.close()` leaves its source-drain loop when replay intake terminates abnormally. G8 adds no rule and does not assign one to G9 without owner direction |
