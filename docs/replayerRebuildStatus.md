...
| G3 | `blocked(PA2)` | The typed producer → Kafka → rebuilt intake → dumper chain is implemented and its core evidence passes. Early final-response retention and unowned-interim ignoring are implemented. The owner resolved an incomplete interim header as ordered ordinary final-response bytes; closure waits on that final implementation/evidence pass |
...
| Typed source-interim protocol | owner decision | core chain and edge designs proved; final ordering implementation pending | Pre-EOM final responses are retained, unowned interims are ignored, and bytes that never complete an interim header remain ordinary final-response writes ordered before exception and close |
...
| Source interim protocol | Typed whole/segmented observations based on PR #3000; ordinary `Write` is never a fallback and old captures get no compatibility decoder | core chain and all edge designs proved; incomplete-header ordering implementation pending | PA2/G3 |
...
|---|---|---|
| G3 closure | blocked(PA2) | The typed producer/consumer chain, early final-response retention, and unowned-interim handling are proved; implement and evidence the final incomplete-header ordering ruling |
...
| Identified but incomplete source interim when the source closes | resolved by owner, 2026-09-25; implementation pending | It is not an interim: emit its bytes as ordinary final-response writes before connection exception and terminal close |
