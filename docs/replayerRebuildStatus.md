...
| G8 | open(final review) | Production is complete through `240678951`: typed revocation and unbounded shutdown grace, generation-scoped force and cleanup, every-predecessor successor gating, independent pause reasons, tracker-gauge conservation, and protocol-violation termination are wired through the real top-level chain. The eight-class focused set passed 105 tests in `/private/tmp/gradle-evidence.19724.15720.log`; exact production compile passed in `/private/tmp/gradle-evidence.19723.31649.log`. The first direct-CLI falsification batch caught 15 valid final-check mutations and exposed three test-fidelity gaps; all three now have strengthened deterministic evidence awaiting exact-revision re-falsification. Claude session `replayer-G8-20260925T231639Z-df3c` returned `NO_ACTIONABLE_FINDINGS` for production through `240678951`; final trace/register review remains |
...
| Compiled test set | proved | G8 focused set: 105 tests, 0 failures, 0 errors, 0 skipped in `/private/tmp/gradle-evidence.19724.15720.log`; the inherited full historical suite remains outside the progress oracle |
...
| G8 first falsification batch | test fidelity | fixed; exact rerun pending | Fifteen final-check mutations failed. Three passing mutations exposed missing assertions for late shutdown assignment in the close gate, every earlier generation rather than one predecessor, and real default-constructor wiring of the one-second grace; all three tests were strengthened |
| G8 conformance review | conformance | open(final pass) | Same-session Claude review fixed shutdown revoke/loss force upgrade, assignment during shutdown, active-generation impossible-input handling, one-second default wiring, cleanup metric semantics, and the G9 `System.exit` handoff note; production re-review returned `NO_ACTIONABLE_FINDINGS`. Final trace/register/evidence pass remains |
| Intake fatality during top-level close | `SILENT`; G9 ownership | deferred(G9) | G8 does not add `ProcessSupervisor` or fatal-shutdown policy. G9 owns supervision and the rule for a failed intake owner while orderly close is waiting |
...
| §17.4 case 28: bootstrap waits behind prior cleanup | G2 | G8 | G8 owns generation cleanup gate | proved — the newest assignment remains paused until every earlier local generation still cleaning up reports exact cleanup |
