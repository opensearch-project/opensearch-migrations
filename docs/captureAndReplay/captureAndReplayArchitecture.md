...
| Kafka input demand and target replay time | With `N = P * T`, where `P` defaults to two requests per target Netty event-loop thread and `T` is fixed at startup, assignment opens one source-local bootstrap entitlement and replay intake recomputes demand over every assigned partition after every input, adding at most one explicit request per generation while fewer than `N` requests have resolved retry-policy source-response input and unfinished target turns; bootstrap and the explicit request may produce a bounded two-batch overshoot; complete responses usually resolve the input before `B + W`, while slow or missing responses use the deterministic boundary; a request may begin target replay before it is counted; demand, generation-cleanup, and lifecycle pause reasons are independent; `TrafficObservation.ts` schedules target work; `LogAppendTime` resolves the irreversible retry boundary and can thereby change demand | Testcontainers per-partition pause/resume, assignment bootstrap concurrent with the ordinary demand pass, every-input demand recomputation while an empty poll consumes neither entitlement, independent pause reasons, fast complete responses, slow or missing responses crossing the boundary, a target turn finishing before later retry-input resolution, one- and two-batch overshoot, mixed-record requests, overdue target schedules, backward timestamps after the boundary, and target-permit release during a source-response wait |
...
- replay intake recomputes demand over every assigned partition after every input, including
  `PartitionGenerationAssigned`; a retry input that resolves after its request's target turn
  finished does not add that request back to the count; assignment bootstrap and one explicit
  request may overlap; an empty poll consumes neither entitlement; and clearing demand pause never
  clears generation-cleanup or lifecycle pause;
