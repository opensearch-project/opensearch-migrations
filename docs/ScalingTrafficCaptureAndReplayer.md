...
The proxy and replayer publish metrics and opt-in traces through **OpenTelemetry**. The applications are configured to send that instrumentation to OTLP collector endpoints. In ECS, the default collector sidecar sends metrics to CloudWatch; X-Ray trace export is enabled separately. For the vended docker-compose solution, metrics use Prometheus and traces can use Jaeger. None of the configurations in the otel-collector prevent it from being scaled horizontally as the sink systems receiving instrumentations are able to handle instruments from a large number of clients.
