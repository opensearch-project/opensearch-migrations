...
This solution isn’t for every customer, specifically those that need perfect data fidelity between their source and target cluster.  Thankfully, there are other technologies that can provide that, including snapshot and restore and cross-cluster replication (CCR).  Major downsides of those solutions are that they 1) don’t work across multiple versions; 2) they can’t provide the user insights into how the target environment will perform.  Even if we enhance those to solutions to work across multiple versions, the new data won’t be analyzed or indexed in the same way as the source cluster, nor should it be.  New data coming into the target cluster wouldn’t be processed in the same way as it was through the source cluster.  Even if the customer tries to configure the clusters exactly the same, differences in Lucene will have an impact on how the data is indexed and queried.  When a customer is migrating to a new version or with different plugins, data consistency will be impossible to achieve.  Data replication where the data has undergone transformations leave the customer needing some validation tools to answer their main question of “go/no-go”.

As the full migration suite of tools evolve, we can give the user more flexibility to use different data replication schemes alongside the capture and replay.  For example, leveraging CCR could let us map the causality of updates to a new cluster while replaying query traffic at approximately the same time that indices are being synced via CCR.  However, we return to some of the same areas of ill-definition as with the original problem.  If a document update is applied differently between two versions, successive updates will diverge, potentially causing the entire cluster to diverge.

The good news for a migration tool that doesn’t provide strong guarantees on the integrity of data is that customers have already chosen an underlying platform where that guarantee is impossible across versions.  Even within the same version, there are cases where updating documents and changing the number of shards could produce wildly different results.  In other words, the butterfly-effect has always been an issue for changes in Elasticsearch clusters.  Our traffic-replay solution attempts to minimize those effects as much as possible and the validations on the target responses attempt to explain any of those differences that may have been unavoidable.

## Problem Statement

### What a Replay Needs to Do

As proven above, there are going to be differences between the source and cluster.  In fact, finding differences isn’t a defect but one of the main features of the traffic-replay strategy, along with replication.  Users will make their determinations based upon how sensitive their applications are to the changes and how much retooling they may need to do to to accept them.  Therefore, it’s important for these tools to minimize differences as much as possible.
...
Notice that the ordering requirement will contradict the first two requirements that are concerned with time.  If a request can only be replayed after a previous request has been handled, and the previous request is being handled by a system outside of our control, there will be cases where slow responses from the target cluster are greater than the gaps in the original source request stream.  We can either weaken the first or second requirement.  Weakening the second by running more requests in parallel/pipelines could be much harder to reason with.  That would introduce a new variable and causality could occur very differently, creating a large number of differences that would need to be evaluated downstream.  To resolve this contradiction, we can use a new requirement that when the exact pacing cannot be maintained, we can at least keep the requests approximately in-sync.  Since updates between individual nodes within a cluster may take seconds, or much longer when a cluster is under stress, that provides justification for affording requests to have a replay jitter within seconds.
...
We need a “get out of jail free” card to reconfigure which topic(s) we’re publishing and consuming because repartitioning Kafka while there are active connections will cause some connections to be split across multiple partitions.  We have no way of preventing that if there are active connections at the time of a repartitioning.  An engineer on the iconic project recommended moving the accumulation logic of the replayer into Spark to do the group-by and reduce on the Traffic Streams to turn them into channel-correlated request streams.  We’re pretty close to being able to do the right thing with Kafka and making correlation Spark’s problem is making the overall solution more complex.
...
* (Medium LOE) Validate that all of the original requests were captured correctly by matching test-client request activity logs with metrics and outputs.
...
* (Large LOE) Read from the ‘control plane’ topic to resolve which topics may have active records.  For a small number of topics, we can read from all of the topics with different TrafficSources in parallel.  As time goes on and our time cursors advance, we can drop away old traffic sources.  It would be simplest to have one BlockingTrafficSource manage a number of separate Kafka sources and let the rest of the replayer pushback on just a single interface rather than need to divide the replayer N-times.
