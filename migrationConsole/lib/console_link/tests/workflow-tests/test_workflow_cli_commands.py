...
class TestSubmitErrorPaths:
    """Test suite for submit command error handling."""

    @patch('console_link.workflow.services.script_runner.subprocess.run')
    @patch('console_link.workflow.commands.submit.WorkflowConfigStore')
    def test_submit_already_exists_shows_hint(self, mock_store_class, mock_subprocess):
        """Test that AlreadyExists error shows helpful hint."""
        import subprocess
        mock_subprocess.side_effect = subprocess.CalledProcessError(
            1, 'cmd', stdout='', stderr='Error from server (AlreadyExists): workflows.argoproj.io already exists'
        )
        mock_store = Mock()
        mock_store_class.return_value = mock_store
    ...
    @patch('console_link.workflow.services.script_runner.subprocess.run')
    @patch('console_link.workflow.commands.submit.WorkflowConfigStore')
    def test_submit_generic_script_error(self, mock_store_class, mock_subprocess):
        """Test that generic script errors show exit code."""
        import subprocess
        mock_subprocess.side_effect = subprocess.CalledProcessError(
            1, 'cmd', stdout='', stderr='some other error'
        )
        mock_store = Mock()
        mock_store_class.return_value = mock_store
        mock_store.load_config.return_value = WorkflowConfig({'parameters': {'message': 'test'}})
    ...
    @patch('console_link.workflow.commands.submit.WorkflowConfigStore')
    def test_submit_no_config_shows_error(self, mock_store_class):
        """Test that missing config shows helpful error."""
        mock_store = Mock()
        mock_store_class.return_value = mock_store
