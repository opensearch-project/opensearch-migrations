"""Unit tests for console_link.middleware.dlq.

Coverage targets:
  * DlqConfig properties (session_prefix, location_uri)
  * load_config resolution precedence (CLI/env > ConfigMap > deployment default)
  * Both DlqNotConfigured branches (no bucket / no session)
...
import console_link.middleware.dlq as dlq


# ---------- shared fixtures / helpers --------------------------------------

# Names of every env var dlq.load_config looks at — clearing them up-front
...
def no_configmap(monkeypatch):
    ...
    monkeypatch.setattr(dlq, "_read_configmap", lambda key: None)
...
def _config(**overrides) -> dlq.DlqConfig:
    base = dict(bucket="b", prefix="rfs-dlq/", session_id="s1", region=None)
    base.update(overrides)
    return dlq.DlqConfig(**base)


# ---------- DlqConfig -------------------------------------------------------

class TestDlqConfigProperties:
    def test_session_prefix_joins_prefix_and_session(self):
        cfg = _config(prefix="rfs-dlq/", session_id="sess-A")
        assert cfg.session_prefix == "rfs-dlq/session=sess-A/"
    ...
    def test_location_helper_returns_location_uri(self):
        cfg = _config()
        assert dlq.location(cfg) == cfg.location_uri
...
class TestS3ClientFactory:
    def test_uses_region_when_set(self, mocker):
        boto_mock = mocker.patch("console_link.middleware.dlq.boto3.client")
        dlq._s3_client(_config(region="eu-central-1"))
        boto_mock.assert_called_once_with("s3", region_name="eu-central-1")

    def test_no_region_arg_when_unset(self, mocker):
        boto_mock = mocker.patch("console_link.middleware.dlq.boto3.client")
        dlq._s3_client(_config(region=None))
...
class TestLoadConfig:
    ...
    def test_configmap_bucket_used_when_env_unset(self, monkeypatch):
        ...
        monkeypatch.setattr(dlq, "_read_configmap", lambda key: cm.get(key))
        cfg = dlq.load_config()
    ...
    def test_session_override_arg_wins_over_env_and_configmap(self, monkeypatch):
        monkeypatch.setenv("RFS_DLQ_S3_BUCKET", "b")
        monkeypatch.setenv("RFS_DLQ_SESSION_ID", "from-env")
        monkeypatch.setattr(dlq, "_read_configmap",
                            lambda key: "from-cm" if key == "session_id" else None)
        cfg = dlq.load_config(session_override="from-arg")
        assert cfg.session_id == "from-arg"

    def test_session_env_wins_over_configmap(self, monkeypatch):
        monkeypatch.setenv("RFS_DLQ_S3_BUCKET", "b")
        monkeypatch.setenv("RFS_DLQ_SESSION_ID", "env-sess")
        monkeypatch.setattr(dlq, "_read_configmap",
    ...
    def test_prefix_env_overrides_default(self, monkeypatch, no_configmap):
        monkeypatch.setenv("RFS_DLQ_S3_BUCKET", "b")
        monkeypatch.setenv("RFS_DLQ_S3_PREFIX", "custom-prefix")  # no trailing slash
        cfg = dlq.load_config(session_override="s")
    ...
    def test_prefix_default_when_env_and_configmap_unset(self, monkeypatch, no_configmap):
        monkeypatch.setenv("RFS_DLQ_S3_BUCKET", "b")
        cfg = dlq.load_config(session_override="s")
        assert cfg.prefix == "rfs-dlq/"

    def test_region_pulled_from_env(self, monkeypatch, no_configmap):
        monkeypatch.setenv("RFS_DLQ_S3_BUCKET", "b")
        monkeypatch.setenv("RFS_DLQ_S3_REGION", "us-west-2")
        cfg = dlq.load_config(session_override="s")
...
class TestListRecords:
    ...
    def test_limit_caps_returned_records(self, mocker):
        ...
        result = dlq.list_records(_config(), limit=2)
        assert len(result) == 2
...
class TestDeleteSession:
    def test_single_batch_under_1000(self, mocker):
        ...
        assert dlq.delete_session(_config()) == 3
    ...
    def test_zero_objects_calls_no_deletes(self, mocker):
        ...
        assert dlq.delete_session(_config()) == 0
    ...
    def test_batches_at_1000_boundary(self, mocker):
        ...
        assert dlq.delete_session(_config()) == 1500
...
class TestSafeCount:
    def test_returns_count_on_success(self, mocker):
        mocker.patch.object(dlq, "count", return_value=42)
        assert dlq.safe_count(_config()) == 42

    def test_returns_none_on_client_error(self, mocker):
        err = ClientError({"Error": {"Code": "NoSuchBucket", "Message": "x"}}, "ListObjects")
        mocker.patch.object(dlq, "count", side_effect=err)
        assert dlq.safe_count(_config()) is None

    def test_returns_none_on_botocore_error(self, mocker):
        # BotoCoreError requires no args.
        mocker.patch.object(dlq, "count", side_effect=BotoCoreError())
        assert dlq.safe_count(_config()) is None
    ...
    def test_does_not_swallow_unrelated_exceptions(self, mocker):
        ...
        mocker.patch.object(dlq, "count", side_effect=ValueError("boom"))
        with pytest.raises(ValueError):
            dlq.safe_count(_config())
...
class TestReadConfigmap:
    def test_returns_stripped_value(self, mocker):
        mocker.patch.object(dlq, "_fetch_configmap_data",
                            return_value={"session_id": "  s1 \n"})
        assert dlq._read_configmap("session_id") == "s1"

    def test_returns_none_for_missing_key(self, mocker):
        mocker.patch.object(dlq, "_fetch_configmap_data", return_value={})
        assert dlq._read_configmap("absent") is None

    def test_returns_none_for_empty_value(self, mocker):
        mocker.patch.object(dlq, "_fetch_configmap_data", return_value={"session_id": ""})
        assert dlq._read_configmap("session_id") is None

    def test_caches_result_across_calls(self, mocker):
        fetch_mock = mocker.patch.object(dlq, "_fetch_configmap_data",
                                         return_value={"bucket": "b1"})
        assert dlq._read_configmap("bucket") == "b1"
...
class TestFetchConfigmapData:
    ...
    def test_returns_data_on_success(self, monkeypatch):
        ...
        assert dlq._fetch_configmap_data() == {"session_id": "s", "bucket": "b"}
    ...
    def test_returns_empty_dict_when_data_is_none(self, monkeypatch):
        ...
        assert dlq._fetch_configmap_data() == {}
    ...
    def test_falls_back_to_kube_config_when_not_in_cluster(self, monkeypatch):
        ...
        assert dlq._fetch_configmap_data() == {"session_id": "out-of-cluster"}
    ...
    def test_returns_empty_dict_on_generic_exception(self, monkeypatch):
        ...
        # is swallowed too — a DLQ inspection command shouldn't crash on
        # ConfigMap weirdness.
        self._install_fake_kubernetes(monkeypatch)
        import sys
...
class TestIterObjects:
    def test_handles_page_with_no_contents_key(self, mocker):
        ...
        result = list(dlq._iter_objects(_config(), client=client))
